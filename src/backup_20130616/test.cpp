...
void reverse(Lnode *head, Lnode *tail) {
	...
	Lnode * cur = head->next; // Changed by Renjie Weng , Lnode * cur = tail
	while (cur != 0) {
		cout << cur->element << " ";
		cur = cur->next;
	}
...
int main ()
{
	...
	Lnode * node1_ptr = head->next = new Lnode; // Change by Renjie
	node1_ptr->element  = 3;
	...
	Lnode * cur = head->next; // Changed by Renjie , Lnode * cur = node1_ptr;
	while (cur != 0) {
		cout << cur->element << " ";
		cur = cur->next;
	}
	...
	reverse(head, node3_ptr); // Changed by Renjie , reverse(node1_ptr, node3_ptr);
	return 0;
}
